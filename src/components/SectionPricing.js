    ...
    render() {
        ...
        return (
            <section id={_.get(section, 'section_id')} className={'block pricing-block bg-' + _.get(section, 'background') + ' outer'}>
                ...
                <div className="inner">
                    <div className="grid">
                        {_.map(_.get(section, 'pricing_plans'), (pricingPlan, pricingPlanIdx) => (
                            <div key={pricingPlanIdx} className={'cell plan' + (_.get(pricingPlan, 'highlight') ? ' highlight' : '')}>
                                <div className="plan-inside">
                                    <h3 className="plan-name">{_.get(pricingPlan, 'title')}</h3>
                                    {_.get(pricingPlan, 'price') &&
                                    <div className="plan-price">{_.get(pricingPlan, 'price')}</div>
                                    }
